...
*   **Newsletter Management:**
    ...
    *   Create a UI in `src/app/department-dashboard/[departmentId]/newsLetter/page.tsx` to manage newsletter years and events.
        ...
        *   **Add a form in `src/app/department-dashboard/[departmentId]/newsLetter/page.tsx` to add new newsletter events to a specific year.** This form includes input fields for 'Newsletter Name', 'Start Date', 'End Date', and a file input for uploading the PDF. It also includes a dropdown to select the year. The form submission is handled by the `handleAddEvent` function, which calls the `addNewsletterEvent` utility function to add the new event to Firestore, including uploading the PDF to Storage. After successful submission, the form fields are cleared and the newsletters are reloaded to display the new entry. The 'Start Date' and 'End Date' inputs are of type 'date', providing a native date picker interface. **The date values from the date pickers are formatted to 'MM/DD/YYYY' using `date-fns` before being stored in the component's state, and displayed in 'YYYY-MM-DD' format for proper rendering in the input fields. The text color of all input fields in this form is set to black.**
